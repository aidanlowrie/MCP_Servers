#!/usr/bin/env python3
"""
Smart BetterTouchTool ↔ MCP bridge
=================================
A **drop-in replacement** for the earlier `server.py` that exposes
*high-level tools* instead of raw BTT JSON.  The assistant can now say
things like

    add_hotkey(trigger="shift+cmd+k", send_keys="ctrl+right")

and the bridge will translate that into the exact BetterTouchTool
format (key codes, modifier masks, etc.), validate it, and call BTT via
its URL scheme.

Run it the same way as before:

    source .venv/bin/activate
    python smart_btt_bridge.py
"""
...
import logging
import traceback
# Simplified imports for Python 3.13 compatibility
from typing import List, Dict, Any, Union

# Use the same import pattern as the working server.py
...
logger = logging.getLogger("smart_btt_bridge")

# Log Python version and module paths to help with debugging
logger.debug(f"Python version: {sys.version}")
logger.debug(f"Sys path: {sys.path}")
logger.debug(f"Starting Smart BTT Bridge initialization")
...
def add_hotkey(
    ...
    if ctx:
        ctx.info(f"Adding hotkey trigger: {trigger} -> {'sending keys: ' + send_keys if send_keys else 'action: ' + predefined_action}")
...
@mcp.tool()
def add_btt_trigger(trigger_json: str, ctx: Union[Context, None] = None) -> str:
    """
    ...
    if ctx:
        ctx.info(f"Adding new trigger with raw JSON: {trigger_json[:50]}...")
